...
| Control | Verdict |
| --- | --- |
| **Search field** | **Not required.** A2 filters over bounded sets; the driver app has none by ruling (file 11 §8, *deliberately not screens*). The brief's likely-set names "admin search"; **no named screen requires it**. If the station list ever outgrows its filters that is a new decision, not a control this pass may add. **[That decision was taken 2026-08-20: ADR-0013.](../../../docs/adr/0013-charger-finder-redesign.md)** A search field ships on D-01, built as §2.3's field box, with §7.1's placeholder refusal intact and a **leading magnifier glyph** identifying the empty box. Two consequences this pass anticipated and one it did not: the field is a new use rather than a new component class (as predicted); the caret, selection and focus calls become urgent rather than theoretical; and the glyph is a **fourth answer to F18** ("an empty field and a button are the same box"), which offered only the mute field, a 2 px border, or a focus treatment that does not answer it. A leading icon inside the box is [INVENTED] under §0.2 and needs its raise. |
